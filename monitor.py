...
def title_overlap(row):
    ...
    if len(x) == 0 or len(y) == 0:
        return -1
    if len(x.intersection(y)) / min(len(x), len(y)) > 0.5:
        return 1
...
def code_overlap(row):
    ...
    if len(inter) / min(len(x), len(y)) > 0.5:
        return 1
    elif len(inter) / max(len(x), len(y)) == 0:
        return -1
    else:
        return 0
...
def title_num_overlap(row):
    ...
    if len(x) == 0 or len(y) == 0:
        return 0
    inter = x.intersection(y)
    if len(inter) / max(len(x), len(y)) > 0.3:
        return 1
...
@labeling_function
def supportd_aspect_ratio_unmatch(row):
    x, y = apply_to_xy(to_str_lower, row.supported_aspect_ratio_l, row.supported_aspect_ratio_r)
    x, y = find_pattern_xy(x, y, "\d+")
    x, y = set(x), set(y)
    if len(x) == 0 or len(y) == 0:
        return 0
...
@labeling_function
def bright_unmatch(row):
    x, y = apply_to_xy(to_str_lower, row.screen_brightness_l, row.screen_brightness_r)
    x, y = find_pattern_xy(x, y, "\d+")
    x, y = set(x), set(y)
    if len(x) == 0 or len(y) == 0:
        return 0
...
def reso_unmatch(row):
    x, y = apply_to_xy(to_str_lower, row.supported_resolution_l, row.supported_resolution_r)
    x, y = find_pattern_xy(x, y, "\d+")
    x, y = set(x), set(y)
    if len(x) == 0 or len(y) == 0:
        return 0
